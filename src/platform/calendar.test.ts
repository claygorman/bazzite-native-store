...
const entry = (appid: number, releaseEpoch: number): CalendarEntry => ({
  game: {
    appid,
    name: `App ${appid}`,
    capsuleUrl: `https://example.invalid/${appid}.jpg`,
    price: '$9.99',
  },
  releaseEpoch,
})
...
check(
  'labels read MON..SUN with TODAY in the middle',
  empty.map((d) => d.label),
  ['MON', 'TUE', 'WED', 'TODAY', 'FRI', 'SAT', 'SUN'],
)
// Today shows the word and NOT the date — the design's one asymmetry.
check(
  'dates are M/D, and empty for today',
  empty.map((d) => d.date),
  ['8/17', '8/18', '8/19', '', '8/21', '8/22', '8/23'],
)
check(
  'exactly one day is today, at index 3',
  empty.map((d) => d.isToday),
...
check(
  'releases outside the window are dropped',
  windowed.flatMap((d) => d.games.map((g) => g.appid)),
  [202, 203],
)

// A release date of 0 is Steam for "no date announced". It is not 1 Jan 1970.
check(
  'undated releases are dropped',
  buildCalendarDays([entry(301, 0)], now).flatMap((d) => d.games),
  [],
)
check(
  'negative release dates are dropped',
  buildCalendarDays([entry(302, -1)], now).flatMap((d) => d.games),
  [],
)

// The anonymous path unions coming_soon with new_releases, and those shelves overlap.
const duped = buildCalendarDays(
  [entry(401, at(2026, 8, 21, 9)), entry(401, at(2026, 8, 21, 9))],
  now,
)
...
check(
  'dates roll over to 1/1',
  buildCalendarDays([], newYear).map((d) => d.date),
...
check(
  'in-window release passes the pre-filter',
  isWithinCalendarWindow(at(2026, 8, 20, 12), now),
  true,
)
check(
  'one day of slack either side is allowed',
  [
...
check(
  'far-future releases are pre-filtered out',
  isWithinCalendarWindow(at(2027, 3, 1, 12), now),
  false,
)
